...
class TritonPythonModel:
    def initialize(self, args):
        ...
        controlnet = ControlNetModel.from_pretrained(
            f"{self.model_dir}/{self.model_ver}/depth",
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            # uploading sd-image and depth image to s3
            output = _encode(output_image) 
            output_depth=_encode(depth_image) 
            output_key=Path(input_data['output'],key.split('/')[-1])
            depth_key=Path(input_data['output'],"depth_"+key.split('/')[-1])
            put_s3_file(bucket, str(output_key),output)
            put_s3_file(bucket, str(depth_key),output_depth)           
